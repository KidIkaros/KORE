...
pub fn load_safetensors_dir(model_dir: &Path) -> Result<KorefModel, FormatError> {
    ...
    for st_path in &st_files {
        let file_data = std::fs::read(st_path).map_err(|e| {
            FormatError::IoError(format!("Failed to read {}: {}", st_path.display(), e))
        })?;
    ...
    if tensor_count == 0 {
        return Err(FormatError::IoError(
            "No tensors found in safetensors files".into(),
        ));
    }
...
fn safetensor_view_to_f32(
    ...
    match dtype {
        ...
        safetensors::Dtype::BF16 => Ok(data
            ...
            .collect()),
        _ => Err(FormatError::InvalidHeader(format!(
...
pub fn inspect_safetensors(path: &Path) -> Result<HashMap<String, Vec<usize>>, FormatError> {
    let data =
        std::fs::read(path).map_err(|e| FormatError::IoError(format!("Failed to read: {}", e)))?;
